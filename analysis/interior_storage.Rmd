...
tcant_beta <- tcant %>% 
  ...
  mutate(delta_pCO2 = pCO2 - lag(pCO2),
         delta_tcant = tcant - lag(tcant),
         beta = delta_tcant/delta_pCO2,
         delta_pCO2_roll = rollmean(delta_pCO2, k=10, fill = NA),
         delta_tcant_roll = rollmean(delta_tcant, k=10, fill = NA),
         beta_roll = rollmean(beta, k=10, fill = NA)) %>% 
  ungroup()

tcant_beta %>% 
  ggplot() +
  geom_path(aes(year, beta, col = method)) +
  geom_point(aes(year, beta, col = method)) +
  geom_path(aes(year, beta_roll, col = method), size = 2) +
...
tcant_beta %>% 
  filter(year > 1955) %>% 
  ggplot() +
  geom_path(aes(year, delta_pCO2, col = method)) +
  geom_point(aes(year, delta_pCO2, col = method)) +
  geom_path(aes(year, delta_pCO2_roll, col = method), size = 2) +
  coord_cartesian(ylim = c(0,3)) +
  theme(legend.position = "bottom")
...
tcant_beta %>% 
  filter(year > 1955) %>% 
  ggplot() +
  geom_path(aes(year, delta_tcant, col = method)) +
  geom_point(aes(year, delta_tcant, col = method)) +
  geom_path(aes(year, delta_tcant_roll, col = method), size = 2) +
  coord_cartesian(ylim = c(0,3)) +
  theme(legend.position = "bottom")
