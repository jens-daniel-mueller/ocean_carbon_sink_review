...
p <- tcant %>%
  ...
  geom_ribbon(
    data = tcant %>% filter(method %in% c("GCB~models", "Green~\"function\"")),
    alpha = 0.3,
    col = NA
  ) +
  geom_linerange(data = tcant %>% filter(method %in% c("Delta*C*\"*\""))) +
  geom_point(data = tcant %>% filter(method %in% c("Delta*C*\"*\""))) +
  ...
  guides(
    ...
    fill = guide_legend(override.aes = list(fill = c("NA", "black", "steelblue")))
  ) +
...
remove_pattern <- paste(g$grobs[[guide]]$grobs[[guide_lines]]$layout$name[c(4,11,16)],
    collapse = "|")
...
tcant <- tcant %>% 
  mutate(method = str_replace(method, "~", " "),
         ...
         method = str_remove_all(method, "\\*"),
         method = str_replace(method, "DeltaC", "dC*"),
         )
